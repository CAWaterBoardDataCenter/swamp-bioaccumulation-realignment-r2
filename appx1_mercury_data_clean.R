...
# Outline ----
# Cleaning data from Jennifer's SWAMP Dataset pull
      # Adding a column for dry weight (ppm)
# Cleaning data from Data Dashboard
# Troubleshooting issues with cleaning data from my pull from the Data Dashboard
...
# Cleaning data from Jennifer's SWAMP Dataset pull ----
# Note from Jennifer's email: Datawarehouse doesn’t allow you to select Program. I did the pull based on Protocol Code, Stations beginning with “2” and filtering Mercury and Moisture.

# Load file and calculate dry weight ----
...
# Save a csv of the pull dataset to use for ----
write.csv(pull, "mercury_ppm.csv")

# Cleaning data from SWAMP Data Dashboard ----
# The following code is not relevant for the map that was created, but included here for documentation.
...
mercury = read.csv("mercury.csv") %>%
  ...
  # each datasheet has lat/long values formatted with different numbers of values after the decimal point, so we input here designating 6 values for consistency and to avoid merging issues later on
  mutate(TargetLongitude = format(TargetLongitude, nsmall = 6)) %>%
  mutate(TargetLatitude = format(TargetLatitude, nsmall = 6)) %>%
  # create uniqueID column
  unite(col = "uniqueID", c("StationCode", "CommonName", 'SampleYear'), sep = "-", remove = F) %>%
  # rename columns to indicate that these data are from the mercury dataset (otherwise they would be the same names as columns in the moisture dataset)
...
# To run the following code, go back and un-comment lines that remove skin off tissue types
# After filtering out other tissue types, let's check what sites remain:
# unique(dat$StationName)
# # All the above stations are still represented, so we are ok to remove all samples with different tissue types than "skin off"

# # 2. Issues with duplicates due to lat/long formatting in datasets ----
# # To see this issue, comment out lines that include:
# mutate(TargetLongitude = format(TargetLongitude, nsmall = 6)) - or TargetLatitude - for both the mercury and moisture datasets
# 
...
# Aha! Here, I was able to see that some duplicated rows are due to differences in the numbers of values after the decimal place in the latitude and/or longitude columns (you have to double click on a specific cell to see the total number of values). To remedy this, I added lines of code limiting the number of decimal places in these rows to 6, which provides about 4 inches of precision and is plenty for our purposes of mapping the mercury concentration data from R2 (the Bay Area) in Esri ArcGIS Pro.
